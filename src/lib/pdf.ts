...
export async function generateCashClosingPDF(
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ["SUBTOTAL (Servicios - Comisiones)", formatCurrency(session.appointments_total - totalBarberCommission)],
      ["Ingresos por venta de productos", formatCurrency(session.sales_total)],
      ["SUBTOTAL ACUMULADO (Servicios Netos + Productos)", formatCurrency((session.appointments_total - totalBarberCommission) + session.sales_total)],
    ...
    didParseCell(data) {
      if (data.section === "body") {
        ...
        } else if (data.row.index === 3 || data.row.index === 5) {
  ...
  if (compiledBarbersBreakdown.length === 0) {
    ...
    compiledBarbersBreakdown.forEach((barber, index) => {
      ...
      const barberBody: string[][] = [
        ...
        ["Recaudo por Citas (Bruto)", formatCurrency(recaudoTotal)],
        [`Comisión por Servicios (${commissionRate}%)`, formatCurrency(barber.total_commission || 0)],
      ];

      if ((barber.total_payments || 0) > 0) {
        barberBody.push(["Abonos devueltos a Caja (+)", `+${formatCurrency(barber.total_payments)}`]);
      }
      
      barberBody.push(["Vales / Adelantos Total (-)", `-${formatCurrency(barber.total_advances || 0)}`]);
      barberBody.push(["Productos a Cargo / Consumo Barbero (-)", `-${formatCurrency((barber.total_product_revenue || 0) + (barber.total_consignments || 0))}`]);
      barberBody.push(["TOTAL DEDUCCIONES (-)", `-${formatCurrency((barber.total_advances || 0) + (barber.total_product_revenue || 0) + (barber.total_consignments || 0))}`]);
      ...
      barberBody.push(
        ["PAGO NETO AL BARBERO - Sale de CAJA FÍSICA", formatCurrency(netPayoutCash)],
        ["PAGO NETO AL BARBERO - Sale de FONDO DIGITAL", formatCurrency(netPayoutDigital)],
      ...
      autoTable(doc, {
        ...
        didParseCell(data) {
          if (data.section === "body") {
            if (data.row.index > 3 && data.row.index < payoutCashIdx) {
              const text = barberBody[data.row.index][0];
              if (text.includes("(-)")) {
                data.cell.styles.textColor = COLORS.rose;
                if (text.includes("TOTAL DEDUCCIONES")) {
                  data.cell.styles.fontStyle = "bold";
                }
      ...
      let currentSubY = (doc as any).lastAutoTable.finalY + 5;
      
      autoTable(doc, {
        startY: currentSubY,
        head: [["Desglose de Servicios Realizados", "Cantidad"]],
        body: serviceEntries.length > 0 
          ? serviceEntries.map(([name, qty]) => [sanitizeText(name), `${qty}`]) 
          : [["Sin registros", "-"]],
        theme: "plain",
        headStyles: {
          fillColor: COLORS.offWhite,
      ...
      currentSubY = (doc as any).lastAutoTable.finalY + 5;
      
      // Products breakdown sub-table
      const productEntries = barber.products_breakdown || [];
      autoTable(doc, {
        startY: currentSubY,
        head: [["Desglose de Productos Cobrados / A Cargo del Barbero", "Cant.", "Costo Total"]],
        body: productEntries.length > 0
          ? productEntries.map((prod: any) => [
              sanitizeText(prod.name),
              `${prod.quantity}`,
              `-${formatCurrency(prod.unit_price * prod.quantity)}`
            ])
          : [["Sin registros", "-", "-"]],
        theme: "plain",
        headStyles: {
        ...
        columnStyles: {
          ...
          2: { halign: "right", fontStyle: "bold", cellWidth: 30, textColor: COLORS.rose },
        },
        margin: { left: 14, right: 14 },
      });
      ...
      // Advances breakdown sub-table
      const advanceEntries = barber.advances_details || [];
      autoTable(doc, {
        startY: currentSubY,
        head: [["Desglose de Vales y Adelantos (Hora - Detalle)", "Medio", "Monto"]],
        body: advanceEntries.length > 0
          ? advanceEntries.map((vale: any) => [
              `${formatTime(vale.time)} - ${sanitizeText(vale.description)}`,
              vale.method === "cash" ? "Efectivo" : "Digital",
