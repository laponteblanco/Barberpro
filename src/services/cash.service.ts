...
export interface BarberBreakdown {
  ...
  /** Ingresos por productos (bruto) */
  total_product_revenue: number;
  /** Comisión total por productos */
  total_product_commission: number;
  /** Desglose de productos { name, quantity, unit_price, commission } */
  products_breakdown: Array<{ name: string; quantity: number; unit_price: number; commission: number }>;
  ...
  /** Desglose de vales y adelantos */
  advances_details: Array<{ time: string; description: string; method: string; amount: number }>;
...
export async function getCashSessionDetailsById(sessionId: string): Promise<ActiveSessionDetails | null> {
  ...
  const [appointmentsRes, staffRes, salesRes, ledgerRes, expensesRes] = await Promise.all([
    ...
    (adminSupabase as any)
      .from("product_sales")
      .select("quantity, unit_price, total_price, product_id, sold_by, created_at, products!inner(name, stock, cost_price)")
      .eq("tenant_id", tenantId)
    ...
    (adminSupabase as any)
      .from("staff_ledger")
      .select("staff_id, type, amount, description, payment_method, created_at, product_id, product_quantity, products(name, stock, cost_price)")
      .eq("tenant_id", tenantId)
      .gte("created_at", openedAtISO)
      .lte("created_at", closedAtISO),
  ...
  ledgerEntries.forEach((item: any) => {
    ...
    if (item.type === "advance") {
      ...
      b.advances_details.push({
        time: item.created_at,
        description: item.description || "Adelanto / Vale",
        method: ledgerMethod,
        amount: amt
      });
  ...
  // Populate product sales revenue data per barber
  sales.forEach((sale: any) => {
    ...
    const b = breakdownMap.get(staffId)!;
    const revenue = Number(sale.total_price || 0);
    // Asumiendo 0% de comisión por defecto (0 comisión)
    const commission = 0; 

    b.total_product_revenue += revenue;
    b.total_product_commission += commission;
    b.products_breakdown.push({
      name: sale.products?.name || "Desconocido",
  ...
  breakdownMap.forEach((b) => {
    ...
    const totalCommissionEarned = b.total_commission;
    
    if (totalRevenue > 0) {
      const cashRatio = b.total_cash / totalRevenue;
      b.payout_cash = Math.round(totalCommissionEarned * cashRatio * 100) / 100;
      b.payout_digital = Math.round((totalCommissionEarned - b.payout_cash) * 100) / 100;
    } else {
      b.payout_cash = 0;
      b.payout_digital = 0;
    }
    ...
    // Consignments (drinks) and Products consumed by the barber are subtracted from their cash payout.
    let remainingDeductionCash = b.total_advances_cash - b.total_payments_cash + b.total_consignments + b.total_product_revenue;
    let netCash = b.payout_cash - remainingDeductionCash;
    let netDigital = b.payout_digital - b.total_advances_digital + b.total_payments_digital;

    // Balance deductions across funds if one goes negative
    if (netCash < 0 && netDigital > 0) {
      const carryOver = Math.min(Math.abs(netCash), netDigital);
      netDigital -= carryOver;
      netCash += carryOver;
    } else if (netDigital < 0 && netCash > 0) {
      const carryOver = Math.min(Math.abs(netDigital), netCash);
      netCash -= carryOver;
      netDigital += carryOver;
    }

    b.net_payout_cash = Math.max(0, netCash);
    b.net_payout_digital = Math.max(0, netDigital);

    // Legacy field: net expected from cash register (updated to represent total payout: cash + digital)
    b.net_expected_cash = Math.max(0, b.net_payout_cash + b.net_payout_digital);
