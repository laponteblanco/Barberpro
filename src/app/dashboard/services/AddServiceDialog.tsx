...
import { Plus, Loader2, X, Scissors, Clock, DollarSign, Tag, Camera, Image as ImageIcon } from "lucide-react";
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button className="h-11 px-6 rounded-2xl bg-indigo-500 text-white font-black uppercase tracking-widest text-xs flex items-center gap-2 hover:brightness-110 transition-all shadow-xl shadow-indigo-500/20 active:scale-[0.98]">
          ...
          <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between shrink-0 bg-zinc-900/50">
            <div>
              <Dialog.Title className="text-xl font-bold text-white tracking-tight flex items-center gap-3">
                <Scissors className="text-indigo-500 w-5 h-5" /> Crear Servicio
              </Dialog.Title>
              <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1 font-medium">Amplía tu catálogo de cortes y arreglos</p>
            </div>
          ...
          <div className="p-8 overflow-y-auto custom-scrollbar">
            <form onSubmit={handleSubmit} className="space-y-7">
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Imagen del Servicio (Opcional)</p>
              </div>
              ...
              <div className="pt-4">
                ...
                >
                  {loading ? <Loader2 className="animate-spin w-5 h-5" /> : "Publicar Servicio"}
                </button>
              </div>
            </form>
          </div>
