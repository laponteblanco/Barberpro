          ...
          <div className="glass-card rounded-[32px] p-8 border-white/5 bg-zinc-900/20 backdrop-blur-3xl shadow-xl space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {(activeSession.barbers_breakdown || []).map((barber: any) => {
                const comisionServicios = barber.total_commission || 0;
                const totalVales = barber.total_advances || 0;
                const totalProductosACargo = (barber.total_consignments || 0) + (barber.total_product_revenue || 0);

                const totalDeducciones = totalVales + totalProductosACargo;
                const expectedNum = Math.max(0, comisionServicios - totalDeducciones);

                const delivery = barberDeliveries[barber.id] || { actual: formatCurrency(expectedNum), isConfirmed: false };
                  ...
                  >
                    ...
                    <div className="mt-5 space-y-4">
                      ...
                      <div className="space-y-1.5 border-b border-white/5 pb-3 mb-3">
                        ...
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-semibold text-zinc-500 flex items-center gap-1.5">
                            <DollarSign className="w-3.5 h-3.5 text-emerald-400" /> Recaudo Total Citas:
                          </span>
                          <span className="font-bold text-white">{formatCurrency((barber.total_cash || 0) + (barber.total_digital || 0))}</span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-semibold text-zinc-500 flex items-center gap-1.5">
                            <User className="w-3.5 h-3.5 text-primary" /> Comisión por Servicios (+):
                          </span>
                          <span className="font-bold text-primary">{formatCurrency(comisionServicios)}</span>
                        </div>
                        {totalVales > 0 && (
                          <div className="flex justify-between items-center text-xs text-red-400">
                            <span className="font-semibold flex items-center gap-1.5">
                              <ArrowUpRight className="w-3.5 h-3.5" /> Vales / Adelantos en Caja (-):
                            </span>
                            <span className="font-bold">−{formatCurrency(totalVales)}</span>
                          </div>
                        )}
                        {totalProductosACargo > 0 && (
                          <div className="flex justify-between items-center text-xs text-red-400">
                            <span className="font-semibold flex items-center gap-1.5">
                              <ShoppingBag className="w-3.5 h-3.5" /> Productos Consumidos (-):
                            </span>
                            <span className="font-bold">−{formatCurrency(totalProductosACargo)}</span>
                          </div>
                        )}
                        <div className="flex justify-between items-center text-xs border-t border-white/5 pt-1 mt-1">
                          <span className="font-semibold text-zinc-500 flex items-center gap-1.5">
                            <Landmark className="w-3.5 h-3.5 text-zinc-400" /> Utilidad Neta Barbería:
                          </span>
                      ...
                      <div className="space-y-1.5 border-b border-white/5 pb-3">
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-semibold text-zinc-500 flex items-center gap-1.5">
                            <Coins className="w-3.5 h-3.5 text-emerald-400" /> Efectivo de Citas Disponibles:
                          </span>
                        ...
                        <div className="flex justify-between items-center text-xs border-t border-white/5 pt-2 mt-1">
                          <span className="font-bold text-yellow-500">Total a Entregar (Liquidación Final):</span>
                          <span className="font-black text-yellow-500 text-base">{formatCurrency(expectedNum)}</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className={`w-full h-11 pl-9 pr-3 bg-zinc-950 border rounded-xl text-white placeholder:text-zinc-800 outline-none text-sm font-bold disabled:opacity-60 disabled:cursor-not-allowed transition-all ${
                              actualNum > expectedNum || actualNum > activeSession.expected_cash
                                ? "border-red-500 focus:border-red-500/50 focus:ring-1 focus:ring-red-500/50"
                          ...
                          onClick={async () => {
                            try {
                              ...
                              const body = [
                                ...
                                ["Recaudo Total Citas", formatCurrency(recaudoTotal)],
                                ["Comisión por Servicios (+)", formatCurrency(comisionServicios)],
                                ["Comisión por Productos (+)", formatCurrency(comisionProductos)],
                              ];
                              
                              if (totalVales > 0) body.push(["Vales / Adelantos en Caja (-)", `-${formatCurrency(totalVales)}`]);
                              if (totalProductosACargo > 0) body.push(["Productos Consumidos / A Cargo (-)", `-${formatCurrency(totalProductosACargo)}`]);
