  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button className="h-11 px-6 rounded-2xl bg-amber-500 text-white font-bold text-sm flex items-center gap-2 hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20 active:scale-95">
          ...
          <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between shrink-0 bg-zinc-900/50">
            <div>
              <Dialog.Title className="text-xl font-bold text-white tracking-tight flex items-center gap-3">
                <Package className="text-amber-500 w-5 h-5" /> Nuevo Producto
              </Dialog.Title>
              <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1 font-medium">Control de inventario</p>
            </div>
          ...
          <div className="p-8 overflow-y-auto custom-scrollbar">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-7">
              ...
              <div className="pt-4">
                ...
                >
                  {loading ? <Loader2 className="animate-spin w-5 h-5" /> : "Registrar Producto"}
                </button>
              </div>
            </form>
          </div>
