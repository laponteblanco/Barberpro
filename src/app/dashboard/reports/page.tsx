...
import { FinancialEvolution } from "@/components/reports/FinancialEvolution";
import { Suspense } from "react";
import Loading from "../loading";
import { getBIAnalytics, getFinancialEvolution } from "@/services/analytics.service";
...
export default async function ReportsPage({ 
  ...
  searchParams: Promise<{ range?: string, date?: string, year?: string }> 
}) {
  const { range = "month", date, year } = await searchParams;
  ...
  const financialData = role !== "barber" ? await getFinancialEvolution(currentYear) : null;
    ...
    <div className="space-y-10 pb-20 animate-fade-up">
      ...
      <Suspense fallback={<Loading />}>
        <div className="space-y-12">
          ...
          {role !== "barber" && financialData && (
            <section>
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 mb-6 ml-1">Finanzas</h2>
              <FinancialEvolution data={financialData} currentYear={currentYear} />
            </section>
          )}
